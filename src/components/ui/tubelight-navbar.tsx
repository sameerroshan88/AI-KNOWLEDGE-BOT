...
export function NavBar({ items, className }: NavBarProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1 sm:gap-2 bg-white border border-black/10 py-1 px-1 rounded-full shadow-lg max-w-[calc(100vw-2rem)] overflow-x-auto">
        {items.map((item) => {
          ...
          return (
            <Link
              ...
              className={cn(
                "relative cursor-pointer text-xs sm:text-sm font-semibold px-3 sm:px-4 py-2 rounded-full transition-colors whitespace-nowrap shrink-0",
              ...
              {isActive && (
                ...
                >
                  <div className="absolute -top-2 left-1/2 -translate-x-1/2 w-8 h-1 bg-black rounded-t-full">
                    ...
                    <div className="absolute w-4 h-4 bg-black/5 rounded-full blur-sm top-0 left-2" />
                  </div>
                </motion.div>
              )}
            </Link>
